...
export const loadCaptchaEnginge = (
  ...
  for (let i = 0; i < parseInt(length); i++) {
    ...
    drawHorizontalLines(height_num, Math.round(Math.random() * (15 - 12) + 12));
  }
  ...
  function drawHorizontalLines(x, y) {
    ...
    ctx.moveTo(x - 8, y);
    ctx.lineTo(x + 8, y);
    ctx.strokeStyle = "black"; // You can set the color of the line
    ctx.stroke();
...
export class LoadCanvasTemplate extends Component {
  render() {
    ...
    LoadCanvasTemplate_HTML =
      '<div class="d-flex justify-content-start gap-2 align-items-center w-100"><canvas id="canv"></canvas><div><a id="reload_href"  style="cursor: pointer; color: ' +
      reload_color +
      '">' +
      "<i class='fa-solid fa-rotate-right' style='color:#005eff'></i>";
    ("</a></div></div>");
